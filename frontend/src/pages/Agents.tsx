...
import { listAgents, createAgent, updateAgent, deleteAgent, publishAgent, getAgentVersions, rollbackAgent, listModels, listTools, listKBs, listPromptTemplates, OPTIONS_PAGE, type AgentRow, type PromptTemplateRow } from '../api'
import { usePagedList } from '../hooks/usePagedList'
import AgentTemplateFields from '../components/prompt/AgentTemplateFields'

// 智能体管理页：表格分页展示智能体；支持新增/编辑（绑定模型、工具、知识库，提示词手填或从模板生成）、
...
export default function Agents() {
  ...
  const [versionAgent, setVersionAgent] = useState<AgentRow | null>(null)
  ...
  // 表单下拉项：各取前 100 条
  useEffect(() => {
    Promise.all([listModels(OPTIONS_PAGE), listTools(OPTIONS_PAGE), listKBs(OPTIONS_PAGE), listPromptTemplates(OPTIONS_PAGE)])
      .then(([m, t, k, p]) => { setModels(m.items); setTools(t.items); setKBs(k.items); setTemplates(p.items) })
      .catch((e: any) => message.error(e.response?.data?.detail || '加载选项失败'))
  }, [])

  // 新增/编辑共用提交：从模板生成时 system_prompt 传空串由后端渲染；手填时清空模板字段
  const onSubmit = async (values: any) => {
    const { use_template, ...rest } = values
    const payload = use_template
      ? { ...rest, system_prompt: '', prompt_template_id: rest.prompt_template_id, prompt_variables: rest.prompt_variables || {} }
      : { ...rest, prompt_template_id: null, prompt_variables: {} }
    try {
  ...
  const openEdit = (record: AgentRow) => {
    setEditing(record)
    form.setFieldsValue({ ...record, use_template: !!record.prompt_template_id })
    setOpen(true)
  }
  ...
  const openVersions = async (record: AgentRow) => {
    try {
  ...
  const columns = [
    ...
    {
      title: '名称', dataIndex: 'name', render: (v: string, r: AgentRow) => (
        <Space size={4}>
          {v}
          {/* 模板改版不自动传播：提示开发者重新保存以按最新版本渲染 */}
          {r.prompt_template_outdated && <Tag color="orange">模板有新版本</Tag>}
        </Space>
      ),
    },
    ...
    {
      title: '操作', render: (_: unknown, record: AgentRow) => (
        <Space>
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <Modal title={editing ? '编辑智能体' : '新增智能体'} open={open} onCancel={() => setOpen(false)} onOk={() => form.submit()} width={640} destroyOnClose>
        <Form form={form} layout="vertical" onFinish={onSubmit} initialValues={{ use_template: false }}>
