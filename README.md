...
| 智能体 | 提示词（手填或从提示词模板渲染）、模型、工具、知识库组合配置，版本发布与历史版本管理 |
| 提示词模板 | 变量声明与 `{{变量}}` 引用，内容变化自动升版本、可回滚，渲染预览；智能体绑定模板后改版不自动传播、列表标记"模板有新版本" |
| 对话 | SSE 流式输出，展示思考过程、工具调用与引用来源，支持中途中断，多轮记忆自动压缩为摘要 |
| 知识库 | 文档上传到 MinIO，按文件类型差异化分片，pgvector 向量检索与关键词检索混合召回，知识库级与切片级权限控制；十万行级表格可直接从页面导入：列表实时显示进度 / 速度 / 预计剩余，多篇排队，中断后从已入库的切片继续 |
...
| 管理 | 用户与角色（启停、重置密码）、审计日志（多条件筛选、展开明细、导出）、API Key（配额进度、来源白名单、限速）、定时任务（cron 校验、下次运行、立即触发、调度器状态）、系统参数（导入并发与批量、检索召回与重排阈值在页面上改、立刻生效） |
...
- **工作流引擎**：把画布上的工作流 JSON 编译为 LangGraph 状态图执行，支持开始、结束、智能体、工具、条件分支、知识库检索、代码、HTTP、循环、人工审核、并行、汇聚 12 类节点；状态键带 reducer、节点只返回增量，并行分支在同一超步并发执行并由汇聚节点收集输出，图结构在保存与运行前校验；人工审核节点通过 `interrupt()` 暂停，管理端审批后用 `Command(resume=...)` 从断点续跑；节点之间用 `{{node_id.field}}` 引用上游输出。
- **RAG 检索**：向量检索与多关键词检索双路召回，RRF 融合后按词法覆盖率重排并做低分与断崖淘汰；对话前先把用户问题改写为多个子查询分别检索；召回前按切片权限快照硬过滤、重排后逐条鉴权，避免越权看到切片。
- **文档入库流水线**：向量化按并发数并行请求向量服务，与写库流水化（在飞批数 = 并发数 + 缓冲），但写库严格按批次顺序提交 —— 续处理靠"已入库切片数"定起点，乱序会算错缺口；每批提交刷新心跳，进程被杀后重启只续心跳已停的文档，心跳还在跳的说明另有进程在写，不抢。并发数、批大小、每次请求条数在页面「系统参数」可调，每篇文档开始处理时读一次。
